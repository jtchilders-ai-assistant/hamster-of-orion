...
Where:
  Active_Population = Population × (1 - TECH% / 100)
...
Tech_Level_Bonus = floor((Highest_Construction_Tech_Level - 1) / 5)
...
Where:
  ...
  Eco_Restoration_Modifier = 1.0 down to 0.10 (based on reduced industrial waste techs)
```
...
Accumulated_Pollution += Pollution_Deficit × (2 / Eco_Restoration_Modifier)
...
Where Lab_Multiplier is based on Computers tech (e.g., Space Scanner = +0 RP, later techs add +1 RP/scientist, etc. see `research-formulas.md`).
...
**Clarification:** In MOO1 style, the TECH slider determines what percentage of population are scientists vs. workers. The other four sliders (SHIP, DEF, IND, ECO) sum to 100% and divide the *production* generated by workers and factories.
...
# Note: These four sliders must sum to exactly 100%
Effective_SHIP% = SHIP%  # These are percentages of total production
Effective_DEF% = DEF%
Effective_IND% = IND%
Effective_ECO% = ECO%

Ship_BC = Production × (SHIP% / 100)
# etc.
```
...
- SHIP/DEF/IND/ECO sum to exactly 100% and divide the remaining production.
...
function process_planetary_turn(planet, empire):
    ...
    base_pop_output = get_base_pop_output(empire)
    mineral_richness = get_mineral_richness(planet)
    production = (operable_factories * 1.0 + workers * base_pop_output) * racial_mod * mineral_richness
    
    # Step 3: Calculate slider BC allocations (sum of the 4 sliders is 100%)
    ship_bc = production * (planet.sliders.SHIP / 100)
    def_bc = production * (planet.sliders.DEF / 100)
    ind_bc = production * (planet.sliders.IND / 100)
    eco_bc = production * (planet.sliders.ECO / 100)
...
- Planetology TL: 25 (Base pop output 1.25)
- Normal Richness
- Racial Modifier: 1.0 (Hamsters)
- Sliders: SHIP 20%, DEF 10%, IND 30%, ECO 40% (sum to 100%), TECH 30%
...
3. **Production:**
   ...
   - Worker production: 42 × 1.25 × 1.0 = 52.5 BC
   - **Total (Normal Richness): 172.5 BC**

4. **Slider Allocations (of 172.5 BC):**
   - SHIP: 172.5 × 0.20 = 34.5 BC
   - DEF: 172.5 × 0.10 = 17.25 BC
   - IND: 172.5 × 0.30 = 51.75 BC
   - ECO: 172.5 × 0.40 = 69.0 BC
