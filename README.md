...
Open `index.html` in any modern browser (double-click it — no server, build step, or install required). Games autosave every turn; three manual save slots are available from the **Game** menu.
...
Research uses the original model — six fields with slider allocation, base cost = level² with race multipliers (Excellent 60% … Poor 125%), 15% interest capped at your yearly investment, breakthrough odds of +1% per 2% overinvestment, and miniaturization (cost halves per 10 levels; weapon size halves, other devices shrink 25%).
...
The galaxy comes in four sizes with nebulas (warp 1, shields fail), five difficulties, 1–5 opponents, fourteen planetary environments, mineral rich/poor worlds, artifacts, fertile and gaia worlds. Random events include comets, plagues, supernovas, rebellions, space monsters, pirates, derelicts, and more. The High Council convenes once half the galaxy is settled and elects a Master of Orion by two-thirds vote — accept the verdict or defy the united galaxy in a Final War. And the Guardian waits at Orion.
...
| Guinea Pigs | Bulrathi | +20 ground combat |
...
| Mice | Meklar | +2 factory controls, free refits |
| Ferrets | Mrrshan | +4 ship attack, first strike |
| Rats | Psilon | +50% research, wider tech choices |
